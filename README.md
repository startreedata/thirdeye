...
```
mvn -B initialize release:prepare -DdryRun=true
```
Skip the `-DdryRun=true` to prepare the release.
...
The `-e` spits out more context if errors occur.
```
mvn -B -e initialize release:clean release:prepare release:perform
```
