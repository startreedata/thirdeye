...
public class RatioCostFunction implements CostFunction {
   ...
   * SIZE_FACTOR_THRESHOLD_PARAM -> Double.  Any node whose size factor is smaller than this
   * threshold, its cost = 0.
   *
  ...
  public double computeCost(double parentChangeRatio, double baselineValue, double currentValue,
    ...
    Preconditions.checkState(DoubleMath.fuzzyCompare(sizeFactor, 0, epsilon) >= 0,
        "Contribution {} is smaller than 0.", sizeFactor);
    Preconditions.checkState(DoubleMath.fuzzyCompare(sizeFactor, 1, epsilon) <= 0,
        "Contribution {} is larger than 1", sizeFactor);
   ...
   * @param baselineValue the baseline value of the current node.
   * @param currentValue the current value of the current node.
   * @param parentRatio parent's change ratio, which is used to produce a virtual change ratio
   *     for the node.
   ...
   */
  private static double errorWithMissingBaselineOrCurrent(double baseline, double currentValue,
      double parentRatio,
      double sizeFactor) {
