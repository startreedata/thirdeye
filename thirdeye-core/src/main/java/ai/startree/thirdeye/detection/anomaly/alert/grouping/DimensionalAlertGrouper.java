...
/**
 * A grouper that groups anomalies by the specified dimensions. If "RollUp" option is enabled, then
 * the groups that
 * contains only one anomaly will be grouped together with an empty group key. Each group of
 * anomalies could have
 * a list of auxiliary email recipients, which will be append to the global email recipients.
 *
 * An usage example:
 * Assume that we have four anomalies, whose dimensions are enclosed in brackets: a1={D1=G1, D2=V1},
 * a2={D1=G1, D2=V2},
 * a3={D1=G2, D2=V3}, and a4={D1=G3, D2=V4}. We further assume that we want to group by dimension D1
 * with roll up
  ...
  public Map<DimensionMap, GroupedAnomalyResultsDTO> group(
    ...
    if (CollectionUtils.isEmpty(groupByDimensions)) {
      ...
      if (doRollUp) {
        ...
        List<MergedAnomalyResultDTO> groupedAnomalyList = rolledUpGroupedAnomaly
            .getAnomalyResults();
        ...
        while (iterator.hasNext()) {
          ...
          if (CollectionUtils.isNotEmpty(groupedAnomalyResults)
              && groupedAnomalyResults.size() == 1) {
