...
public class TimeOnTimeResponseParser {
  ...
  public List<Row> parseResponse() {
    ...
    if (currentRanges.size() != baselineRanges.size()) {
      LOGGER.info(
          "Current and baseline time series have different length, which could be induced by DST.");
    }
    rows = new ArrayList<>();
  ...
  private void parseGroupByDimensionResponse() {
    ...
    if (includeOther) {
      for (int i = 0; i < numMetrics; i++) {
        otherBuilder
            .addMetric(metricFunctions.get(i).getMetricName(), otherBaseline[i], otherCurrent[i]);
      }
  ...
  private void parseGroupByTimeDimensionResponse() {
    ...
    Map<Integer, List<Double>> baselineMetricSums = ResponseParserUtils
        .getMetricSumsByTime(baselineResponse);
    Map<Integer, List<Double>> currentMetricSums = ResponseParserUtils
        .getMetricSumsByTime(currentResponse);
    ...
    for (String dimensionValue : dimensionValues) {
      ...
      for (int timeBucketId = 0; timeBucketId < numTimeBuckets; timeBucketId++) {
        ...
        String baselineTimeDimensionValue = ResponseParserUtils
            .computeTimeDimensionValue(timeBucketId, dimensionValue);
