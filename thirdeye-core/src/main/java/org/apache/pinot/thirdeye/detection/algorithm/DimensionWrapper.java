...
public class DimensionWrapper extends DetectionPipeline {
  ...
  public DimensionWrapper(DataProvider provider, AlertDTO config, long startTime, long endTime) {
    ...
    this.minContribution = MapUtils
        .getDoubleValue(config.getProperties(), "minContribution", Double.NaN);
    this.minValue = MapUtils.getDoubleValue(config.getProperties(), "minValue", Double.NaN);
    this.minValueHourly = MapUtils
        .getDoubleValue(config.getProperties(), "minValueHourly", Double.NaN);
    this.maxValueHourly = MapUtils
        .getDoubleValue(config.getProperties(), "maxValueHourly", Double.NaN);
    this.minValueDaily = MapUtils
        .getDoubleValue(config.getProperties(), "minValueDaily", Double.NaN);
    this.maxValueDaily = MapUtils
        .getDoubleValue(config.getProperties(), "maxValueDaily", Double.NaN);
    ...
    this.lookback = ConfigUtils
        .parsePeriod(MapUtils.getString(config.getProperties(), "lookback", "1w"));
    this.timezone = DateTimeZone
        .forID(MapUtils.getString(config.getProperties(), "timezone", "America/Los_Angeles"));
    ...
    this.liveBucketPercentageThreshold = MapUtils
        .getDoubleValue(config.getProperties(), "liveBucketPercentageThreshold", 0.5);

    // the metric to run the detection for
    this.nestedMetricUrns = ConfigUtils.getList(config.getProperties().get(PROP_NESTED_METRIC_URNS),
    ...
    this.nestedMetricUrnKey = MapUtils.getString(config.getProperties(), PROP_NESTED_METRIC_URN_KEY,
        PROP_NESTED_METRIC_URN_KEY_DEFAULT);
  ...
  private List<MetricEntity> dimensionExplore() {
    ...
    if (this.metricUrn != null) {
      ...
      MetricConfigDTO metricConfig = this.provider
          .fetchMetrics(Collections.singleton(metric.getId())).get(metric.getId());
      MetricSlice slice = MetricSlice
          .from(metric.getId(), this.start.getMillis(), this.end.getMillis(), metric.getFilters());
      ...
      DataFrame aggregates = this.provider
          .fetchAggregates(Collections.singletonList(slice), this.dimensions, limit).get(slice);

      if (aggregates.isEmpty()) {
        return nestedMetrics;
      }

      final double total = aggregates.getDoubles(DataFrame.COL_VALUE).sum().fillNull()
          .doubleValue();

      // min contribution
      if (!Double.isNaN(this.minContribution)) {
        aggregates = aggregates.filter(
            aggregates.getDoubles(DataFrame.COL_VALUE).divide(total).gte(this.minContribution))
            .dropNull();
      }
      ...
      if (!Double.isNaN(this.minValueHourly)) {
        aggregates = aggregates.filter(
            aggregates.getDoubles(DataFrame.COL_VALUE).multiply(hourlyMultiplier)
                .gte(this.minValueHourly)).dropNull();
      }

      if (!Double.isNaN(this.maxValueHourly)) {
        aggregates = aggregates.filter(
            aggregates.getDoubles(DataFrame.COL_VALUE).multiply(hourlyMultiplier)
                .lte(this.maxValueHourly)).dropNull();
      }

      if (!Double.isNaN(this.minValueDaily)) {
        aggregates = aggregates.filter(
            aggregates.getDoubles(DataFrame.COL_VALUE).multiply(dailyMultiplier)
                .gte(this.minValueDaily)).dropNull();
      }

      if (!Double.isNaN(this.maxValueDaily)) {
        aggregates = aggregates.filter(
            aggregates.getDoubles(DataFrame.COL_VALUE).multiply(dailyMultiplier)
                .lte(this.maxValueDaily)).dropNull();
      }
    ...
    if (!Double.isNaN(this.minLiveZone) && !Double.isNaN(this.minValue)) {
      ...
      nestedMetrics = nestedMetrics.stream().filter(metricEntity -> checkMinLiveZone(metricEntity))
  ...
  public DetectionPipelineResultV1 run() throws Exception {
    ...
    if (CacheConfig.getInstance().useCentralizedCache() && !CacheConfig.getInstance()
        .useInMemoryCache()) {
      ...
      Map<DatasetConfigDTO, List<MetricEntity>> nestedMetricsByDataset = nestedMetrics.stream()
          .collect(Collectors.groupingBy(metric -> datasetToConfigMap
                  .get(metricIdToConfigMap.get(metric.getId()).getDataset()),
              Collectors.toList()));

      long cachingPeriodLookback =
          config.getProperties().containsKey(PROP_CACHE_PERIOD_LOOKBACK) ? MapUtils
              .getLong(config.getProperties(),
                  PROP_CACHE_PERIOD_LOOKBACK) : DEFAULT_CACHING_PERIOD_LOOKBACK;
      // prefetch each
      for (Map.Entry<DatasetConfigDTO, List<MetricEntity>> entry : nestedMetricsByDataset
          .entrySet()) {
        // if cachingPeriodLookback is set in the config, use that value.
        // otherwise, set the lookback period based on data granularity.
        long metricLookbackPeriod = cachingPeriodLookback < 0 ? ThirdEyeUtils
        ...
        this.provider.fetchTimeseries(entry.getValue()
            .stream()
            .map(metricEntity -> MetricSlice
                .from(metricEntity.getId(), startTime - metricLookbackPeriod, endTime,
    ...
    for (int i = 0; i < totalNestedMetrics; i++) {
      ...
      LOG.info("running detection for metric urn {}. {}/{}", metric.getUrn(), i + 1,
          totalNestedMetrics);
  ...
  private void warmUpAnomaliesCache(List<MetricEntity> nestedMetrics) {
    ...
    Multimap<AnomalySlice, MergedAnomalyResultDTO> warmUpResults = this.provider
        .fetchAnomalies(Collections.singleton(anomalySlice));
    LOG.info("Warmed up anomalies cache for detection {} with {} anomalies - start: {} end: {}",
        config.getId(),
  ...
  private List<EvaluationDTO> calculateEvaluationMetrics(List<PredictionResult> predictionResults) {
    return predictionResults.stream().filter(
        predictionResult -> this.evaluationMetricUrns.contains(predictionResult.getMetricUrn()))
        .map(prediction -> EvaluationDTO
            .fromPredictionResult(prediction, this.startTime, this.endTime,
                this.config.getId()))
        .collect(Collectors.toList());
  }

  private void checkEarlyStop(long totalNestedMetrics, long successNestedMetrics, int i,
    ...
    long earlyStopThreshold = Math
        .max(EARLY_STOP_THRESHOLD, (int) (totalNestedMetrics * EARLY_STOP_THRESHOLD_PERCENTAGE));
   ...
   * still allow the detection to finish and return the generated anomalies. Because the data
   ...
   * @param predictions the prediction results generated, which can tell us whether there are
   *     detectors run successfully.
   * @throws DetectionPipelineException the exception to throw
   */
  private void checkNestedMetricsStatus(long totalNestedMetrics, long successNestedMetrics,
      List<Exception> exceptions,
    ...
    if (successNestedMetrics == 0 && totalNestedMetrics > 0) {
      ...
      if (exceptions.stream()
          .allMatch(e -> e.getCause() instanceof DetectorDataInsufficientException)
          && predictions.size() != 0) {
        LOG.warn(
  ...
  private boolean checkMinLiveZone(MetricEntity me) {
    MetricSlice slice = MetricSlice
        .from(me.getId(), this.start.getMillis(), this.end.getMillis(), me.getFilters());
