...
public class OnboardResourceHttpUtils extends AbstractResourceHttpUtils {
  ...
  public String getClonedFunctionID(long functionId, String tag, boolean isCloneAnomaly)
      throws IOException {
    HttpPost req = new HttpPost(
        ONBOARD_RESOURCE_ENDPOINT + FUNCTION_KEY + "/" + functionId + "/" + CLONE_KEY + "/"
            + tag + "?cloneAnomaly=" + isCloneAnomaly);
    return callJobEndpoint(req);
  }

  public String removeMergedAnomalies(long functionId, long startTime, long endTime)
      throws IOException {
    HttpPost req = new HttpPost(ONBOARD_RESOURCE_ENDPOINT + FUNCTION_KEY + "/" + functionId + "/"
        + DELETE_EXISTING_ANOMALIES
        + "?start=" + startTime
        + "&end=" + endTime);
    return callJobEndpoint(req);
  }
