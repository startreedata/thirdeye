...
public class MetricCorrelationRankingPipeline extends Pipeline {
  ...
  public PipelineResult run(PipelineContext context) {
    ...
    LOG.info("Processing {} target metrics and {} candidate metrics", targetMetrics.size(),
        candidateMetrics.size());
    ...
    for (MetricEntity entity : allMetrics) {
      ...
      DataFrame current = new DataFrame(responses.get(currentId))
          .renameSeries(COL_VALUE, COL_CURRENT);
      DataFrame baseline = new DataFrame(responses.get(baselineId))
          .renameSeries(COL_VALUE, COL_BASELINE);
    ...
    for (MetricEntity targetMetric : targetMetrics) {
      ...
      for (MetricEntity candidateMetric : candidateMetrics) {
        ...
        try {
          ...
          LOG.debug("Score for target '{}' and candidate '{}' is {} (based on {} data points)",
              targetMetric.getUrn(), candidateMetric.getUrn(), score, joined.size());
          ...
          LOG.warn("Could not calculate correlation of target '{}' and candidate '{}'. Skipping.",
              targetMetric.getUrn(), candidateMetric.getUrn(), e);
        }
      }
    }
    ...
    for (MetricEntity me : scores.keySet()) {
      entities
          .add(me.withScore(scores.get(me)).withRelated(new ArrayList<Entity>(related.get(me))));
    }
  ...
  private List<TimeSeriesRequestContainer> makeRequests(Collection<MetricEntity> metrics,
      TimeRangeEntity range, Multimap<String, String> filters, String prefix) {
