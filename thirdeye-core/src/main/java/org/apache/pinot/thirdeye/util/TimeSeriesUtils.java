...
public class TimeSeriesUtils {
   ...
   */
  public static AnomalyTimelinesView toAnomalyTimeLinesView(TimeSeries observed,
      TimeSeries expected, long bucketMillis) {
  ...
  /**
   * Convert the given anomaly time lines view to a tuple of TimeSeries, the first one is observed,
   * the second one is expected
   */
  public static Tuple2<TimeSeries, TimeSeries> toTimeSeries(
    ...
    TreeMap<TimeBucket, Tuple2<Double, Double>> sortedView = new TreeMap<>(
        new Comparator<TimeBucket>() {
          @Override
    ...
    for (int i = 0; i < timeBuckets.size(); i++) {
      sortedView
          .put(timeBuckets.get(i), new Tuple2<>(observedValues.get(i), expectedValues.get(i)));
    }
    ...
    observed.setTimeSeriesInterval(
        new Interval(timeBuckets.get(0).getCurrentStart(),
