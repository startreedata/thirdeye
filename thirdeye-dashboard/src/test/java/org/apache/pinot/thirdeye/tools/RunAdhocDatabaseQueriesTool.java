...
public class RunAdhocDatabaseQueriesTool {
  ...
  private void disableAllActiveDetections(Collection<Long> excludeIds) {
    List<AlertDTO> detections = detectionConfigDAO.findAll();
    for (AlertDTO detection : detections) {
  ...
  private void cleanup() {
    List<AlertDTO> alertDTOS = detectionConfigDAO.findAll();

    for (AlertDTO detFunction : alertDTOS) {
    ...
    for (SubscriptionGroupDTO subscription : subscriptions) {
      ...
      if (vectorClocks != null && vectorClocks.size() != 0) {
        for (Map.Entry<Long, Long> clockEntry : vectorClocks.entrySet()) {
          AlertDTO alertDTO = detectionConfigDAO.findById(clockEntry.getKey());
          if (alertDTO == null) {
          ...
          Predicate predicate = Predicate.AND(
              ...
              Predicate.EQ("detectionConfigId", alertDTO.getId()));
