...
import org.apache.pinot.thirdeye.detection.detectors.MeanVarianceRuleDetectorSpec;
...
public class MeanVarianceRuleDetectorTest {
  ...
  public void setUp() throws Exception {
    ...
    timeseries.put(MetricSlice
            .from(1L, 1499126400000L, 1562630400000L, filters, TimeGranularity.fromString("7_DAYS")),
        this.data);
  ...
  public void testComputePredictedTimeSeriesDaily() {
    ...
    Assert
        .assertEquals(Math.round(timeSeries.getPredictedBaseline().getDouble(0) * 1000.0) / 1000.0,
            174431.234);
    Assert
        .assertEquals(Math.round(timeSeries.getPredictedBaseline().getDouble(1) * 1000.0) / 1000.0,
            147184.878);
    Assert
        .assertEquals(Math.round(timeSeries.getPredictedBaseline().getDouble(2) * 1000.0) / 1000.0,
            153660.665);
  }
  ...
  public void testWeekOverWeekLookbackChange() {
    ...
    DetectionResult detectionResult = detector
        .runDetection(new Interval(1530576000000L, 1562630400000L), "thirdeye:metric:1");
