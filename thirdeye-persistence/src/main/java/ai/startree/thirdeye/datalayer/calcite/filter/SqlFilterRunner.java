...
public class SqlFilterRunner<T> {
  ...
  public List<T> applyFilter(final List<T> elements, @NotNull final String queryFilter) {
    ...
    } catch (RelConversionException | SQLException | SqlParseException | ValidationException e) {
  ...
  private static ResultSet runQuery(final Planner planner, final String sqlQuery)
      throws SqlParseException, ValidationException, RelConversionException, SQLException {
    final SqlNode sqlNode = planner.parse(sqlQuery);
