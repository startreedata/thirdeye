...
public class AnomalyMergerPostProcessorTest {
  ...
  public void testPrepareSortedAnomalyList() {
    assertThat(AnomalyMergerPostProcessor.combineAndSort(emptyList(), emptyList())).isEqualTo(emptyList());
    ...
    assertThat(AnomalyMergerPostProcessor.combineAndSort(emptyList(), singletonList(existing1))).isEqualTo(
        singletonList(existing1));

    assertThat(AnomalyMergerPostProcessor.combineAndSort(singletonList(new1), emptyList())).isEqualTo(
        singletonList(new1));
  ...
  public void testEmptyMergeAndSave() {
    final AnomalyMergerPostProcessorSpec spec = new AnomalyMergerPostProcessorSpec()
        .setUsage(DetectionPipelineUsage.DETECTION);
    final AnomalyMergerPostProcessor merger = new AnomalyMergerPostProcessor(spec);
    assertThat(merger.mergeAnomalies(emptyList())).isEqualTo(List.of());
  }
...
//    final List<MergedAnomalyResultDTO> output = anomalyMergerPostProcessor.merge(newAlert(),
...
//        plusMin(JANUARY_1_2021_01H, 10));
//
//    final List<MergedAnomalyResultDTO> sorted = anomalyMergerPostProcessor.combineAndSort(List.of(new1, new2),
//        List.of(existing1, existing2));
//
//    final List<MergedAnomalyResultDTO> merged = anomalyMergerPostProcessor.doMerge(sorted);
...
//
//    final List<MergedAnomalyResultDTO> sorted = anomalyMergerPostProcessor.combineAndSort(List.of(new1),
//        List.of(existing1));
//
//    final List<MergedAnomalyResultDTO> merged = anomalyMergerPostProcessor.doMerge(sorted,
