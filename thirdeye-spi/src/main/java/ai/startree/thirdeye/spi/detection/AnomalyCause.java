/*
 * Copyright 2023 StarTree Inc
 *
 * Licensed under the StarTree Community License (the "License"); you may not use
 * this file except in compliance with the License. You may obtain a copy of the
 * License at http://www.startree.ai/legal/startree-community-license
 *
 * Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT * WARRANTIES OF ANY KIND,
 * either express or implied.
 * See the License for the specific language governing permissions and limitations under
 * the License.
 */
package ai.startree.thirdeye.spi.detection;

public enum AnomalyCause {
  HOLIDAY_EFFECT("Holiday effect"),

  USER_ERROR("User error"),

  SOFTWARE_FAILURE("Software failure"),

  ENVIRONMENTAL_FACTOR("Environmental factors (like Storm, EQ ... )"),

  FRAUD("Fraudulent activities"), 
  
  PLATFORM_UPGRADE("Platform upgrades"),

  EXTERNAL_EVENT("External events (Ex: Campaigns, Price changes etc)"),

  OTHER("Others");

  final String userReadableName;

  AnomalyCause(String userReadableName) {
    this.userReadableName = userReadableName;
  }

  public String getUserReadableName() {
    return this.userReadableName;
  }
}
